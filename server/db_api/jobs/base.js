module.exports = function(app, db) {
  ...
  var module = {
            ...
            connection.query("SELECT error, error_code FROM jobs WHERE user_id = ? AND id = ?", [user_id, job_id],
              function(err, docs) {
                if (err) {
                  ...
                  var jobInfo = docs[0];
                  if (jobInfo) {
                    if (jobInfo.error && (jobInfo.error_code == "TimeoutInterrupt" ||
                        jobInfo.error_code == "ExternalInterrupt" ||
                        jobInfo.error_code == "UserReportedInterrupt")) {

                      callback(false, jobInfo.error_code);
                    } else {
                      callback(false, false);
                    }
                  } else {
    ...
    userReportedInterruptAllRunningJobsOnLander: function(user, lander_id, callback) {
      var user_id = user.id;

      var userReportedInterruptAllJobsWithLanderId = function() {
        app.log("\nExternal User Interrupt called on all jobs with lander_id : " + lander_id, "debug");

        var query = "";
        var arr = [];
        //also include deployLanderToDomain jobs if we're undeploying
        query = "UPDATE jobs SET error = ?, error_code = ?, processing = ? WHERE user_id = ? AND lander_id = ? AND done = ?";
        arr = [true, "UserReportedInterrupt", false, user_id, lander_id, false];
      ...
      userReportedInterruptAllJobsWithLanderId();
    },
