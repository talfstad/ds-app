module.exports = function(app, db) {
  ...
  var deleteLander = function(user, attr) {
      ...
      if (err) {
        ...
        db.landers.getAllDomainsLanderIsDeployedOn(user, lander_id, function(err, dbDomains) {
          if (err) {
            ...
            var deleteOldDeployedS3Dir = function(folderPathToDelete, callback) {
            ...
            var undeployDomain = function(folderPathToDelete, cloudfront_id, callback) {
              deleteOldDeployedS3Dir(folderPathToDelete, function(err) {
                if (err) {
                  callback(err);
                } else {
