...
<td>
  <%= deploy_status_gui %> 
      ...
      <div style="float: right; margin-right: 10px">
        ...
        <% if(active_campaigns_gui.length > 0 && deploy_status === "deployed") { %>

          <% _.each(active_campaigns_gui, function(campaignName, i){ %>
          
          <a class="campaign-tab-link" href="#">
            <span><%= campaignName %></span></a><% if (i !== active_campaigns_gui.length-1){ %>,<% } %>
          <% }); %>
