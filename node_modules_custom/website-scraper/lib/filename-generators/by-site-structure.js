...
function getSafeAbsoluteFilename (urlPath){
    ...
    safeFileName = safeFileName.replace(/\?.*/g, ''); //remove from ? to end of url
    safeFileName = safeFileName.replace(/\.php$/, ".html"); //set page to html if was php
	safeFileName = path.normalize(safeFileName);
	return decodeURIComponent(safeFileName);
}
